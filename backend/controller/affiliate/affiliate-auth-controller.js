  ...
  // unique referral code generation
        generateUniqueReferralCode((referralCode) => {
          const insertSql = `
            INSERT INTO affiliates (name, email, password, phone, referral_code, status, join_date)
            VALUES (?, ?, ?, ?, ?, 'active', NOW())`;

          connection.query(insertSql, [name, email, hashedPassword, phone, referralCode], (err, result) => {
  ...
  const getProfile = (req, res) => {
    ...
    const sql = "SELECT id AS affiliate_id, name, email, referral_code, bank_info FROM affiliates WHERE id = ?";
