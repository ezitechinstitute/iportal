...
function SendMailOnsite(name, email) {
  let year = new Date().getFullYear();
  let transporter = nodeMailer.createTransport({
    host: process.env.MAILONSITEHOST, // Your SMTP server host
    port: process.env.MAILONSITEPORT, // Your SMTP server port (default is usually 587)
    secure: true, // true for 465, false for other ports
    auth: {
      user: process.env.ONSITEMAIL, // Your email address
      pass: process.env.ONSITEPASS, // Your email password or application-specific password
    },
  });

  let mailOptions = {
    from: process.env.ONSITEMAIL, // Sender email address
    to: email, // List of recipients
    subject: "Ezitech Onsite Internship", // Subject line
    // Plain text body
...
module.exports = { SendMailRemote, SendMailOnsite };
