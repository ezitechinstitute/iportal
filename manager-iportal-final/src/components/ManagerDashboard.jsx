...
export const ManagerDashboard = () => {
  ...
  const GetManagerAmount = async () => {
    try {
      const res = await axios.get(
        ...
        `http://localhost:8000/get-manager-amount/${managerEmail}`,
        { headers: { 'x-access-token': token } }
      );
      if (
        res.data &&
        res.data.length > 0 &&
        res.data[0].balance !== undefined
      ) {
        setAmount(res.data[0].balance);
      } else {
        setAmount(0); // Set default value if data is not available
      }
    } catch (error) {
      console.log(error);
  ...
  const GetChartData = async () => {
    try {
      //'https://api.ezitech.org/get-statics'
      const res = await axios.get('http://localhost:8000/get-statics', {
        headers: { 'x-access-token': token },
      });
      const { onsite, remote } = res.data || { onsite: [], remote: [] };
