...
import { FiMenu, FiPower, FiUser } from 'react-icons/fi'; // Importing React Icons
    ...
    const fetchProfileImage = async () => {
      ...
      try {
        const response = await axios.get(
          `https://api.ezitech.org/manager/profile/${managerId}`,
          {
          ...
          <div className='bookmark-wrapper d-flex align-items-center'>
            <ul className='nav navbar-nav d-xl-none'>
              <li className='nav-item'>
                <Link className='nav-link menu-toggle' href='#'>
                  <FiMenu className='ficon' />{' '}
                  {/* Replaced data-feather="menu" */}
                </Link>
              </li>
            </ul>
          </div>
            ...
            <li className='nav-item dropdown dropdown-user'>
              ...
              >
                ...
                <a
                  ...
                  onClick={Logout}
                >
                  <FiPower className='mr-50' />{' '}
                  {/* Replaced data-feather="power" */}
                  Logout
                </a>
              </div>
            </li>
